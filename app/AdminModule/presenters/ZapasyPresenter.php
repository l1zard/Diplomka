...
class ZapasyPresenter extends BasePresenter
{
    ...
    {
        $this->template->_form = $this['addMatch'];
    }

    // HANDLES

    public function handleLigaChange($value, $type)
    {
        if ($type == 1) {
            if ($value) {
                $array = $this->leagueModel->getKlubByLeagueIdArray($value);
                $this['addMatch']['klubDomaci']->setPrompt('Vyberte klub')
                    ->setItems($array);
                $this['addMatch']['klubHoste']->setPrompt('Vyberte klub')
                    ->setItems($array);
            }
            else{
                $this['addMatch']['klubDomaci']->setPrompt('Vyberte klub')
    ...
    protected function createComponentAddMatch()
    {
        ...
        $form->addSelect('klubDomaci', "Vyber klub")
            ->setPrompt('Zvolte nejdříve ligu')
            ->setAttribute('class', 'form-control');
        $form->addSelect('klubHoste', "Vyber klub")
            ->setPrompt('Zvolte nejdříve ligu')
            ->setAttribute('class', 'form-control');
        $form->addText('datum')
        ...
        $form->addText('kolo')
            ->setAttribute('class', 'form-control');
        $form->addSubmit('send', 'Přidat zápas')
            ->setAttribute('class', 'btn btn-success right');
        $form->onSuccess[] = $this->addMatchSucceed;

        return $form;
    }

    public function addMatchSucceed(UI\Form $form)
    {
        $values = $form->getHttpData();
        if($values['klubDomaci'] == $values['klubHoste']){
            $this->flashMessage("Nelze zvolit dva stejné týmy!", 'alert-danger');
            $this->redirect('this');
        }
        else{
            $this->matchModel->addMatch($values['liga'], $values['klubDomaci'], $values['klubHoste'], $values['datum'], $values['kolo'], $values['informace']);
            $this->flashMessage('Zápas byl přidán', 'alert-success');
    ...
    public function actionDetail($id)
    {
        ...
        $this->template->isShowed = $this->matchModel->getMatchShow($this->idMatch);
